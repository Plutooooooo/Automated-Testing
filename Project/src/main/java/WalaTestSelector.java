...
public class WalaTestSelector {
    ...
    private static void initWalaInfo(String targetPath) throws IOException, InvalidClassFileException, ClassHierarchyException, CancelException {
        ...
        classesCalledByTestClass = walaAnalysis.recordClassesCalledByTestClass(classesCalledByTest);
    }
    ...
    private static Set<String> selectTestsOnClassLevel() {
        ...
        for (String key : classesCalledByTestClass.keySet()) {
            ...
            System.out.println("testclass is "+ key);
            ...
            if (flag) {
                //该测试类下的所有方法都要选中
                for(String testMethod : methodsUnderTestClass.get(key)){
                    res.add(key+" "+testMethod);
                }
            }
        }
        return res;
